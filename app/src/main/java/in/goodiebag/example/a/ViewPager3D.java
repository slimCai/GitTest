package in.goodiebag.example.a;
...
public class ViewPager3D extends ViewPager {
	...
	protected boolean getChildStaticTransformation(View child, Transformation transformation) {
		...
		if (mOverscrollEffect.isOverscrolling() && isFirstOrLast) {
			...
			mCamera.getMatrix(transformation.getMatrix());
			mCamera.restore();
			transformation.getMatrix().preTranslate(-dx, -dy);
			transformation.getMatrix().postTranslate(dx, dy);

			if (mAnimateAlpha) {
				transformation.setTransformationType(Transformation.TYPE_BOTH);
				transformation.setAlpha((float) Math.sin((float) ((1 - Math.abs(mOverscrollEffect.mOverscroll)) * Math.PI / 2)));
			}
			return true;
			...
			if (mTempTect.left >= mScrollPositionOffsetPixels) {
				if (mAnimateAlpha) {
					transformation.setTransformationType(Transformation.TYPE_BOTH);
					transformation.setAlpha((float) Math.sin((float) (mScrollPositionOffset
							* Math.PI / 2)));
				}
				// right side
